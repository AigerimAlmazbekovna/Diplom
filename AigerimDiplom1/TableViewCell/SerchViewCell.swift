...
class SearchViewCell: UITableViewCell {
    ...
    private lazy var nameLabel: CustomLabel = {
        let label = CustomLabel(text: "Алмазбекова Айгерим", Fontname: FontTextType.medium.rawValue, Fontsize: 16, UIColorhexRGB: ColorType.LabelTextColor.textBlackColor.rawValue, lineHeightMultiple: 1.24, kern: 0.16)
    ...
    public func setupCell(user: User) {
        ...
        nameLabel.text = "\(user.firstName ?? NSLocalizedString("Имя", comment: "")) \( user.secondName ?? NSLocalizedString("Фамилия", comment: ""))"
        professionLabel.text = user.profession ?? NSLocalizedString("профессия", comment: "")
    }
