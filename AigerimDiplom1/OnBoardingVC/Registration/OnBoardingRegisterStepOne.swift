...
class OnBoardingRegisterStepOne: UIViewController {
    ...
    private lazy var textTitleLabel: CustomLabel = {
        let label = CustomLabel(text: NSLocalizedString("ЗАРЕГИСТИРОВАТЬСЯ", comment: ""), Fontname: FontTextType.medium.rawValue, Fontsize: 22, UIColorhexRGB: ColorType.LabelTextColor.textBlackColor.rawValue, lineHeightMultiple: 0, kern: 0.18)
        return label
    }()
    
    private lazy var phoneLabel: CustomLabel = {
        let label = CustomLabel(text: NSLocalizedString("Введите номер", comment: ""), Fontname: FontTextType.bold.rawValue, Fontsize: 24, UIColorhexRGB: ColorType.LabelTextColor.textLightColor.rawValue, lineHeightMultiple: 1.24, kern: 0.16)
        return label
    }()
    
    private lazy var descriptionLabel: CustomLabel = {
        let label = CustomLabel(text: NSLocalizedString("Ваш номер будет использоваться для входа в аккаунт", comment: ""), Fontname: FontTextType.regular.rawValue, Fontsize: 12, UIColorhexRGB: ColorType.LabelTextColor.textDescriptionColor.rawValue, lineHeightMultiple: 1.03, kern: -0.17)
    ...
    private lazy var nextButton: CustomButton = {
        let button = CustomButton(title: NSLocalizedString("ДАЛЕЕ", comment: ""),
    ...
    private lazy var descriptionBottonLabel: CustomLabel = {
        let label = CustomLabel(text: NSLocalizedString("Нажимая кнопку Далее Вы принимаете пользовательское Соглашение и политику конфедициальности ", comment: ""), Fontname: FontTextType.regular.rawValue, Fontsize: 12, UIColorhexRGB: ColorType.LabelTextColor.textDescriptionColor.rawValue, lineHeightMultiple: 1.03, kern: -0.17)
        label.numberOfLines = 0
        label.alpha = 0
        label.textAlignment = .center
        return label
    ...
    @objc private func tapingButton(sender: UIButton) {
        coreDataManager.chekcUser(for: myPhoneNumber) { [self] (user) in
            if user != nil {
                ...
                let alertController = UIAlertController(title: NSLocalizedString("ОШИБКА ВХОДА", comment: ""), message: "Данный номер \(myPhoneNumber) уже зарегистирован", preferredStyle: .alert)
